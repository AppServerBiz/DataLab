...
// Altura do card expandida novamente para caber os dados Quantitativos (Fator de Lucro e DD)
int base_card_w = 280; 
int base_card_h = 375;
int base_head_h = 230; 
...
struct SMagicData
  {
   ...
   // Novas Variáveis Analíticas Quantitativas
   double   gross_profit;
   double   gross_loss;
   double   max_dd_money; // Rebaixamento máximo histórico deste robô específico
...
void LoadLimitsFromCSV()
  {
   ...
   int handle = FileOpen(InpLimitsFile, FILE_CSV|FILE_READ|FILE_ANSI, ';');
   if(handle != INVALID_HANDLE)
     {
      ...
      FileClose(handle);
      Print("Nautilus V2.08: ", ArraySize(robot_limits), " Limites carregados.");
     }
  }
...
void CollectData()
  {
   ...
   for(int j=0; j<ArraySize(magics); j++)
     {
      magics[j].gross_profit = 0;
      magics[j].gross_loss = 0;
      magics[j].max_dd_money = 0;
      ...
      double magic_run_bal = 0;
      double magic_peak = 0;

      // Percorre do mais antigo para o mais novo para calcular o Max Drawdown Real
      for(int i=0; i<totalDeals; i++) {
         ...
         if(HistoryDealGetInteger(t, DEAL_MAGIC) == magics[j].magic) {
            ...
            if(entry == DEAL_ENTRY_OUT || entry == DEAL_ENTRY_OUT_BY) {
               ...
               // Cálculo Qualitativo
               if(p > 0) magics[j].gross_profit += p;
               else magics[j].gross_loss += MathAbs(p);
               
               // Curva de Capital e Drawdown Máximo do Robô
               magic_run_bal += p;
               if(magic_run_bal > magic_peak) magic_peak = magic_run_bal;
               double current_dd = magic_peak - magic_run_bal;
               if(current_dd > magics[j].max_dd_money) magics[j].max_dd_money = current_dd;
      ...
      double current_total_lots = magics[j].buy_lots + magics[j].sell_lots;
      int current_total_entries = magics[j].buy_count + magics[j].sell_count;
      
      for(int idx=0; idx<ArraySize(robot_limits); idx++) {
         if(robot_limits[idx].magic == magics[j].magic) {
            if((robot_limits[idx].max_lots > 0 && current_total_lots > robot_limits[idx].max_lots) || 
               (robot_limits[idx].max_entries > 0 && current_total_entries > robot_limits[idx].max_entries)) {
...
void UpdateUI()
  {
   ...
   CreateLabel(pref+"Tit", head_x+25, y0, "Supervision Nautilus v2.08", InpClrProfit, font_xxl);
   ...
   for(int i=0; i<cards_found; i++)
     {
      ...
      // NOVOS DADOS QUANTITATIVOS (Fator de Lucro e Max DD Histórico)
      int oy = ty + (int)(118*InpScale);
      
      double pf = (magics[i].gross_loss > 0) ? (magics[i].gross_profit / magics[i].gross_loss) : magics[i].gross_profit;
      string pf_str = (magics[i].gross_loss == 0 && magics[i].gross_profit == 0) ? "0.00" : DoubleToString(pf, 2);
      
      CreateLabel(pref+"C_PF_L_"+id, x+15, oy, "Fator Lucro:", InpClrText, font_m);
      CreateLabel(pref+"C_PF_V_"+id, x+(int)(95*InpScale), oy, pf_str, InpClrTextHl, font_m);
      
      CreateLabel(pref+"C_DD_L_"+id, x+15, oy+(int)(16*InpScale), "Max DD Hist:", InpClrText, font_m);
      CreateLabel(pref+"C_DD_V_"+id, x+(int)(95*InpScale), oy+(int)(16*InpScale), "-"+DoubleToString(magics[i].max_dd_money, 2), InpClrLoss, font_m);
      
      // BLOCO DE STATUS EXISTENTE EMPURRADO PARA BAIXO
      oy += (int)(42*InpScale);
