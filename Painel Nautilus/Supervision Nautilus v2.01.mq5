...
// Larguras e Alturas base - Ajustado largura do Card para caber mais do nome
int base_card_w = 295; 
int base_card_h = 280;
int base_head_h = 230; 
   ...
   for(int j=0; j<ArraySize(magics); j++)
     {
      ...
      // Simulação Eixo 2 (Lotes Anômalos)
...
void UpdateUI()
  {
   ...
   if(dd_ratio >= 0.75) { risk_color = InpClrRiskRed; risk_label = "CRÍTICO"; }
   else if(dd_ratio >= 0.50) { risk_color = InpClrRiskOrange; risk_label = "CONTENÇÃO"; }
   else if(dd_ratio >= 0.25) { risk_color = InpClrRiskYellow; risk_label = "INVESTIGAÇÃO"; }
   ...
   // Borda do cabeçalho agora com 2px visuais de impacto
   ...
   // Linha 1: Título Principal e Status Global (Badge style string)
   CreateLabel(pref+"Tit", head_x+25, y0, "Supervision Nautilus v2.01", InpClrProfit, font_xxl);
   
   string risk_text = "STATUS: " + risk_label + "  |  DD: " + DoubleToString(current_dd_pct, 2) + "% / DME: " + DoubleToString(InpMaxDME, 0) + "%";
   CreateLabel(pref+"GlobalRisk", right_col_x, y0, risk_text, risk_color, font_l, ANCHOR_RIGHT_UPPER); // Usando font_l para elegância
   
   // Linha 2: Conta | Flutuante
   int y1 = y0 + gap_tit_acc;
   string accName = AccountInfoString(ACCOUNT_NAME);
   CreateLabel(pref+"AccL", head_x+25, y1, "Conta: " + accName + "  |  Pico: " + DoubleToString(peak_ref, 2), InpClrText, font_m);
   ...
   CreateLabel(pref+"FloatV", right_col_x, y2, DoubleToString(floatTot, 2)+" USD", floatTot>=0?InpClrProfit:InpClrLoss, font_xl, ANCHOR_RIGHT_UPPER);
   ...
   CreateLabel(pref+"StatsL", head_x+25, y3, "Robôs: "+IntegerToString(ArraySize(magics))+"   |   DataBase: "+dbDate, InpClrText, font_m);
   
   CreateLabel(pref+"Bal", right_col_x, y3, "Saldo Bruto: "+DoubleToString(current_bal, 2), InpClrTextHl, font_m, ANCHOR_RIGHT_UPPER);
   ...
   // Espaçamento dos Lucros no Topo - Melhor distribuído
   int x_off = head_x + 25;
   int col_w = (int)((head_w_calc - 300) / 4); // Distribui proporcionalmente
   CreateLabel(pref+"G_Dia", x_off, y4, "Dia: "+DoubleToString(g_prof_d, 2)+" ("+DoubleToString(pct_d, 2)+"%)", g_prof_d>=0?InpClrProfit:InpClrLoss, font_m);
   x_off += col_w; 
   CreateLabel(pref+"G_Sem", x_off, y4, "Semana: "+DoubleToString(g_prof_w, 2)+" ("+DoubleToString(pct_w, 2)+"%)", g_prof_w>=0?InpClrProfit:InpClrLoss, font_m);
   x_off += col_w;
   CreateLabel(pref+"G_Mes", x_off, y4, "Mês: "+DoubleToString(g_prof_m, 2)+" ("+DoubleToString(pct_m, 2)+"%)", g_prof_m>=0?InpClrProfit:InpClrLoss, font_m);
   x_off += col_w;
   CreateLabel(pref+"G_Tot", x_off, y4, "Total: "+DoubleToString(g_prof_t, 2)+" ("+DoubleToString(pct_t, 2)+"%)", g_prof_t>=0?InpClrProfit:InpClrLoss, font_m);
   ...
   for(int i=0; i<cards_found; i++)
     {
      ...
      if(magics[i].floating < 0 && magics[i].floating < -(peak_ref * (InpMaxDME/100.0) * 0.25)) // > 25% DME culpa de um robô
      ...
      if(StringLen(title) > 38) title = StringSubstr(title, 0, 35) + "..."; // Aumentado limite de caracteres
      ...
      CreateLabel(pref+"C_BuyL_"+id, x+15, oy, "Lotes C:  "+DoubleToString(magics[i].buy_lots, 2), InpClrText, font_m);
      CreateLabel(pref+"C_SelL_"+id, x+(int)(110*InpScale), oy, "Lotes V:  "+DoubleToString(magics[i].sell_lots, 2), InpClrText, font_m);
      
      // Alerta do Eixo 2 (Lotes) - Bem mais limpo e posicionado
      int oy_warn = oy+(int)(25*InpScale);
      if(magics[i].lotes_anomalos) {
         CreateLabel(pref+"C_WarnL_"+id, x+15, oy_warn, "Status:", InpClrText, font_m);
         CreateLabel(pref+"C_WarnV_"+id, x+(int)(60*InpScale), oy_warn, "⚠️ DESVIO CRÍTICO", InpClrRiskYellow, font_m);
      } else {
         CreateLabel(pref+"C_WarnL_"+id, x+15, oy_warn, "Status:", InpClrText, font_m);
         CreateLabel(pref+"C_WarnV_"+id, x+(int)(60*InpScale), oy_warn, "✔ Normal", InpClrRiskGreen, font_m);
      }

      CreateLabel(pref+"C_FloatL_"+id, x+15, oy+(int)(55*InpScale), "Flutuante:", InpClrText, font_m);
      CreateLabel(pref+"C_FloatV_"+id, x+card_w-15, oy+(int)(55*InpScale), DoubleToString(magics[i].floating, 2) + " USD", magics[i].floating>=0?InpClrProfit:InpClrLoss, font_l, ANCHOR_RIGHT_UPPER); // Usando font_l pro Flutuante do robô
     }
   ...
   CreateRect(pref+"FootBox", footer_center_x - footer_box_w/2, footer_y, footer_box_w, footer_box_h, InpClrBg, InpClrBg); // Fundo transparente pro footer
