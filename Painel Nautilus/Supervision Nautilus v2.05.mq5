...
void UpdateUI()
  {
   ...
   // Linha 1: Título Principal (Agora com Hyperlink)
   CreateLabel(pref+"Tit", head_x+25, y0, "Supervision Nautilus v2.05", InpClrProfit, font_xxl);
   ...
   // Fonte Gigante para o Semáforo Global (Aumentado 1.5x) e posição recuada (850)
   int font_huge = (int)(52 * InpScale); // 1.5x maior que a versão v2.04 (~35)
   int bx_g = right_col_x - (int)(850 * InpScale);
   // Ajustado espaço entre bolhas para o novo tamanho
   ...
   for(int i=0; i<cards_found; i++)
     {
      ...
      // Card Color Highlight Revisitado: Calculando o ratio individual primeiro!
   ...
   // Retirado rodapé totalmente! Não há mais nada renderizado embaixo dos botões
   ChartRedraw();
  }
