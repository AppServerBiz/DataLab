...
struct SMagicData
  {
   ...
   // Eixo 2 Controles
   int      status_eixo2; // 0=OK, 1=Cuidado 80%, 2=Estourado 100%
...
void LoadLimitsFromCSV()
  {
   ...
   // Mudança Crucial: Usando FILE_TXT ao invés de FILE_CSV para leitura bruta por linha!
   ...
   if(handle != INVALID_HANDLE)
     {
         ...
         if(k >= 3)
           {
            ...
            // Troca virgulas por ponto caso o usuario use Excel PT-BR
      ...
      FileClose(handle);
      Print("Nautilus V2.09: ", ArraySize(robot_limits), " Limites de Robôs carregados com sucesso do arquivo: ", InpLimitsFile);
     }
   else
     {
      Print("Nautilus V2.09 Aviso: Arquivo '", InpLimitsFile, "' não encontrado na pasta MQL5\\Files. Nenhuma restrição do Eixo 2 ativa.");
     }
  }
   ...
   for(int j=0; j<ArraySize(magics); j++)
     {
      ...
      for(int idx=0; idx<ArraySize(robot_limits); idx++) {
         if(robot_limits[idx].magic == magics[j].magic) {
            ...
            double lots_ratio = (robot_limits[idx].max_lots > 0) ? current_total_lots / robot_limits[idx].max_lots : 0;
            double entries_ratio = (robot_limits[idx].max_entries > 0) ? (double)current_total_entries / robot_limits[idx].max_entries : 0;
            
            // Logica "UM OU OUTRO" (O pior cenário prevalece)
            double max_ratio = MathMax(lots_ratio, entries_ratio);
            ...
            if(max_ratio >= 1.0) {
                ...
                magics[j].status_eixo2 = 1; // Yellow (80%)
                magics[j].lotes_anomalos = false;
...
void UpdateUI()
  {
   ...
   CreateLabel(pref+"Tit", head_x+25, y0, "Supervision Nautilus v2.09", InpClrProfit, font_xxl);
   ...
   for(int i=0; i<cards_found; i++)
     {
      ...
      color c_lote_t = InpClrText; 
      color c_lote_warn = InpClrText;
      string s_lote_warn = "Desvio de Lotes: Normal";
      
      // Dinâmica Visual de Cores para Lotes - Baseada no Eixo 2
      if(magics[i].status_eixo2 == 2) {
          c_lote_t = InpClrRiskRed;
          c_lote_warn = InpClrRiskRed;
          s_lote_warn = "⚠️ DESVIO CRÍTICO DE ORDENS/LOTES";
      } else if (magics[i].status_eixo2 == 1) {
          c_lote_t = InpClrRiskYellow;
          c_lote_warn = InpClrRiskYellow;
          s_lote_warn = "⚠️ ATENÇÃO: APROXIMANDO DO LIMITE (80%)";
      } else if (magics[i].limit_max_lots > 0 || magics[i].limit_max_entries > 0) {
          c_lote_t = InpClrText;
          c_lote_warn = InpClrRiskGreen;
          s_lote_warn = "Lotes sob controle (Planilha OK)";
      } else {
          c_lote_t = InpClrText;
          c_lote_warn = InpClrText;
          s_lote_warn = "Sem limite configurado";
      }

      CreateLabel(pref+"C_BuyL_"+id, x+15, oy, "Lotes Compra: "+DoubleToString(magics[i].buy_lots, 2)+" | "+IntegerToString(magics[i].buy_count)+"x", c_lote_t, font_m);
      CreateLabel(pref+"C_SelL_"+id, x+15, oy+(int)(20*InpScale), "Lotes Venda: "+DoubleToString(magics[i].sell_lots, 2)+" | "+IntegerToString(magics[i].sell_count)+"x", c_lote_t, font_m);
