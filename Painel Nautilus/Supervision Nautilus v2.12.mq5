...
int base_card_w = 280; 
int base_card_h = 340; // Voltamos à altura compacta 
int base_head_h = 230; 
   ...
   if(handle != INVALID_HANDLE)
     {
      ...
      FileClose(handle);
      Print("Nautilus V2.12: ", ArraySize(robot_limits), " Limites de Robôs carregados com sucesso do arquivo: ", InpLimitsFile);
     }
   else
     {
      Print("Nautilus V2.12 Aviso: Arquivo '", InpLimitsFile, "' não encontrado na pasta MQL5\\Files. Nenhuma restrição do Eixo 2 ativa.");
     }
  }
   ...
   for(int j=0; j<ArraySize(magics); j++)
     {
      ...
      for(int idx=0; idx<ArraySize(robot_limits); idx++) {
         if(robot_limits[idx].magic == magics[j].magic) {
            ...
            // Alterado tolerâncias e matemática (0.999 = limite total), (0.699 = 70%)
...
void UpdateUI()
  {
   ...
   CreateLabel(pref+"Tit", head_x+25, y0, "Supervision Nautilus v2.12", InpClrProfit, font_xxl);
   ...
   for(int i=0; i<cards_found; i++)
     {
      ...
      // Ajuste: InpClrLoss para Vermelho e InpClrRiskYellow para Amarelo
      color c_buy_t = InpClrText; 
      if(magics[i].buy_status == 2) c_buy_t = InpClrLoss;
      else if(magics[i].buy_status == 1) c_buy_t = InpClrRiskYellow;

      color c_sell_t = InpClrText;
      if(magics[i].sell_status == 2) c_sell_t = InpClrLoss;
