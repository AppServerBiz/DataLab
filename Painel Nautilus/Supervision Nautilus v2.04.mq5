...
// Aumentada altura novamente (340) para dar mais fólego visual e afastar colisão
int base_card_w = 280; 
int base_card_h = 340;
int base_head_h = 230; 
...
void UpdateUI()
  {
   ...
   CreateLabel(pref+"Tit", head_x+25, y0, "Supervision Nautilus v2.04", InpClrProfit, font_xxl);
   ...
   // Fonte Gigante para o Semáforo Global e posição bem à Esquerda
   int font_huge = (int)(35 * InpScale); // O dobro do tamanho de uma xxl base praticamente
   int bx_g = right_col_x - (int)(750 * InpScale); // Empurrado MUITO pra esquerda antes da frase
   
   CreateLabel(pref+"GSem1", bx_g, y0-8, "●", g_c_verde, font_huge);   bx_g += (int)(35*InpScale);
   CreateLabel(pref+"GSem2", bx_g, y0-8, "●", g_c_amarela, font_huge); bx_g += (int)(35*InpScale);
   CreateLabel(pref+"GSem3", bx_g, y0-8, "●", g_c_laranja, font_huge); bx_g += (int)(35*InpScale);
   CreateLabel(pref+"GSem4", bx_g, y0-8, "●", g_c_verm, font_huge);
   ...
   for(int i=0; i<cards_found; i++)
     {
      ...
      // Porcentagem do Flutuante (comparado ao Saldo Bruto)
      ...
      // SEMÁFORO DE RISCO POR ROBÔ - AFUNDADO E AFASTADO PARA DIREITA
      ...
      int bx = x + (int)(120*InpScale); // Afastado MUITO para a direita
   ...
   // Rodapé com fundo transparente retirando a borda Header
   CreateRect(pref+"FootBox", footer_center_x - footer_box_w/2, footer_y, footer_box_w, footer_box_h, InpClrBg, InpClrBg);
