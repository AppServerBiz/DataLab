...
// Voltando a largura do Card para 280. Aumentando levemente a altura para caber o semáforo.
int base_card_w = 280; 
int base_card_h = 310;
int base_head_h = 230; 
...
struct SMagicData
  {
   ...
   int      buy_count, sell_count; // NOVO: Quantidade de Ordens (Preços Médios)
   double   floating;
   bool     lotes_anomalos;
  };
...
void UpdateUI()
  {
   ...
   // Linha 1: Título Principal e Status Global
   CreateLabel(pref+"Tit", head_x+25, y0, "Supervision Nautilus v2.02", InpClrProfit, font_xxl);
   
   string risk_text = "STATUS: " + risk_label + "  |  DD: " + DoubleToString(current_dd_pct, 2) + "% / DME: " + DoubleToString(InpMaxDME, 0) + "%";
   CreateLabel(pref+"GlobalRisk", right_col_x, y0, risk_text, risk_color, font_l, ANCHOR_RIGHT_UPPER);
   ...
   // RETORNADO PARA A VERSÃO ANTERIOR DE ESPAÇAMENTO (V2.00)
   int x_off = head_x + 25;
   ...
   for(int i=0; i<cards_found; i++)
     {
      ...
      if(StringLen(title) > 35) title = StringSubstr(title, 0, 32) + "..."; // Voltando ao limite de v2.00
      ...
      // RETORNANDO PARA A LINHA VERTICAL COMO V2.00 E ADICIONANDO CONTAGEM NUMÉRICA (# Ordens)
      int oy = ty + (int)(125*InpScale);
      CreateLabel(pref+"C_BuyL_"+id, x+15, oy, "Lotes Compra: "+DoubleToString(magics[i].buy_lots, 2)+" | "+IntegerToString(magics[i].buy_count), InpClrText, font_m);
      CreateLabel(pref+"C_SelL_"+id, x+15, oy+(int)(20*InpScale), "Lotes Venda: "+DoubleToString(magics[i].sell_lots, 2)+" | "+IntegerToString(magics[i].sell_count), InpClrText, font_m);
      
      // Alerta do Eixo 2 (Lotes) estilo v2.00
      ...
      // NOVO: SEMÁFORO DE RISCO POR ROBÔ (Bolinhas Visuais)
      int oy_semaf = oy+(int)(80*InpScale);
      ...
      // Acendendo conforme impacto individual do robô no DME
      ...
      // O verde sempre fica acesso para indicar que o módulo está rastreando
      
      int bx = x + (int)(95*InpScale);
      // Letra Webdings com bolas redondas sólidas = 159 (Mas Segoe UI usa Arial/Unicode, bola = ●)
   ...
   CreateRect(pref+"FootBox", footer_center_x - footer_box_w/2, footer_y, footer_box_w, footer_box_h, InpClrBg, InpClrBg); // Mantido o rodapé limpo transparente
