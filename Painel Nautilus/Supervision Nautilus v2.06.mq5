...
void LoadLimitsFromCSV()
  {
   ...
   if(handle != INVALID_HANDLE)
     {
      // Formato esperado das linhas: MagicNumber;LotesMaximos;EntradasMaximas
      // Ex: 96134388;0.50;5
      while(!FileIsEnding(handle))
        {
         string line = FileReadString(handle);
         if(StringLen(line) < 3) continue; // Pula linha vazia
         string parts[];
      ...
      FileClose(handle);
      Print("Nautilus V2.06: ", ArraySize(robot_limits), " Limites de Robôs carregados do arquivo: ", InpLimitsFile);
     }
   else
     {
      Print("Nautilus V2.06 Aviso: Arquivo '", InpLimitsFile, "' não encontrado na pasta MQL5\\Files. O alerta de Eixo 2 não ativará de forma real sem este arquivo.");
     }
  }
...
void TriggerAlert(string title, string msg)
  {
   if(InpEnableAlerts) Alert(title, ": ", msg);
...
void CollectData()
  {
   // Guarda histórico de alertas para não apagar ao re-inicializar Array
   ...
   for(int j=0; j<ArraySize(magics); j++)
     {
      ...
      // EIXO 2 ATIVO: Checagem Real no CSV
         ...
         if(robot_limits[idx].magic == magics[j].magic) {
            if((robot_limits[idx].max_lots > 0 && current_total_lots > robot_limits[idx].max_lots) || 
               (robot_limits[idx].max_entries > 0 && current_total_entries > robot_limits[idx].max_entries)) {
                ...
                // Dispara o PUSH Notification se ainda não avisou
                if(!magics[j].already_alerted) {
                   TriggerAlert("EIXO 2 - DESVIO DE LOTES", "Robô #" + IntegerToString(magics[j].magic) + " excedeu os limites do Manual! Lotes/Entradas atuais: " + DoubleToString(current_total_lots,2) + " (" + IntegerToString(current_total_entries) + "). Limite estabelecido: " + DoubleToString(robot_limits[idx].max_lots,2) + " (" + IntegerToString(robot_limits[idx].max_entries) + ").");
                   magics[j].already_alerted = true;
                }
            } else {
               magics[j].already_alerted = false; // Reseta se voltar ao normal
            }
            break;
         }
      }
...
void UpdateUI()
  {
   ...
   // Lógica Disparo Push Global Eixo 1: Apenas quando muda de faixa subindo
   if(current_global_status > last_global_risk_level) {
       TriggerAlert("EIXO 1 - " + risk_label, "Conta: " + IntegerToString(acctNumber) + "\nRebaixamento Global: " + DoubleToString(current_dd_pct, 2) + "%\nDME Rompido: " + DoubleToString(dd_ratio*100.0, 0) + "%");
   }
   ...
   CreateLabel(pref+"Tit", head_x+25, y0, "Supervision Nautilus v2.06", InpClrProfit, font_xxl);
...
int OnInit() { 
    ...
    LoadLimitsFromCSV(); // <--- Leitura Real da Tabela do Eixo 2
    UpdateUI(); 
    EventSetTimer(1); 
