...
# 1. Build da imagem
docker build -t $REGISTRY_URL/$APP_NAME:v1 .

# 2. Login (se configurou auth no Caddy)
docker login $REGISTRY_URL -u admin -p '$REGISTRY_PASS'

# 3. Push para o seu servidor
docker push $REGISTRY_URL/$APP_NAME:v1

# 4. Criar diretório remoto (se não existir)
ssh -i $SSH_KEY ubuntu@164.152.61.174 "sudo mkdir -p /opt/micro-saas/apps/$APP_NAME && sudo chown ubuntu:ubuntu /opt/micro-saas/apps/$APP_NAME"

scp -i $SSH_KEY docker-compose.yml ubuntu@164.152.61.174:/opt/micro-saas/apps/$APP_NAME/docker-compose.yml

# 5. Login no registry dentro do servidor remoto
ssh -i $SSH_KEY ubuntu@164.152.61.174 "echo '$REGISTRY_PASS' | sudo docker login $REGISTRY_URL -u admin --password-stdin"
...
ssh -i $SSH_KEY ubuntu@164.152.61.174 "grep -q '$CF_RECORD' /opt/micro-saas/caddy/Caddyfile || printf '\n$CF_RECORD {\n    reverse_proxy $APP_NAME:3000\n}\n' | sudo tee -a /opt/micro-saas/caddy/Caddyfile"

# 7. Recarregar Caddy
ssh -i $SSH_KEY ubuntu@164.152.61.174 "sudo docker exec -w /etc/caddy caddy_proxy caddy reload"

# 8. Verificar se o app está rodando
ssh -i $SSH_KEY ubuntu@164.152.61.174 "sudo docker ps | grep $APP_NAME"
